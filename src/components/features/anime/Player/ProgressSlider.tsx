...
const ProgressSlider: React.FC<ProgressSliderProps> = ({
  ...
  return (
    ...
    >
      <div className={classNames(`${isMobile ? 'w-full h-4 relative' : `w-full h-3 relative`}`, innerClassName)}>
        ...
        {state?.timestamps?.map((ts, index) => {
          ...
          return (
            <Slider.Bar
              className="h-4 w-4 absolute bg-primary-700 rounded-full"
              style={{
                left: `${startPercent}%`,
        ...
        <Slider.Bar className="bg-primary-600" percent={currentPercent} />
        <Slider.Bar className="bg-white/10" />

        {!hideDot && (
          <Slider.Dot
            className="h-4 w-2 absolute bg-slate-50 p-2 rounded-full"
