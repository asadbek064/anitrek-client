...
const Landing = () => {
  ...
  return (
    <div className=" text-white py-24 px-4 flex flex-col">
      ...
      <div className="w-full my-16 flex justify-center flex-col md:flex-row space-y-4 md:space-x-4 md:space-y-0">
            <Link href={"/browse?type=anime"} passHref>
                <Button className="[font-size:var(--step-0)] tracking-wide font-semibold py-6 px-8 rounded outline outline-1 outline-sky-600" primary>{t("button-search")}</Button>
            </Link>

            <Link href={"/register"} passHref>
                <Button className="[font-size:var(--step-0)] tracking-wide font-semibold py-6 px-8 rounded outline outline-1" secondary>{t("button-register")}</Button>
            </Link>
      </div>
    </div>
  );
